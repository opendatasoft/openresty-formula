...
def installed(name, user=None, version=None):
    """
    ...
    try:
        __salt__["opm.install"](name, runas=user, version=version)
        ret["result"] = True
...
def removed(name, user=None):
    ...
    if name not in __salt__["opm.list"](runas=user):
        ret["result"] = True
    ...
    if __salt__["opm.remove"](name, runas=user):
        ret["result"] = True
